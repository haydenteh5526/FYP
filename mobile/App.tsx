...
export default function App() {
  ...
  useEffect(() => {
    AsyncStorage.getItem('token').then(t => { setToken(t); setLoading(false); });
  }, []);
  ...
  async function handleCapture() {
    ...
    setLoading(true);
    try {
      await uploadDocument(result.assets[0].uri, 'scan.jpg');
  ...
  return (
    <View style={styles.authContainer}>
      ...
      <TouchableOpacity onPress={() => setIsLogin(!isLogin)}>
        <Text style={styles.link}>{isLogin ? 'Create account' : 'Sign in instead'}</Text>
      </TouchableOpacity>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  button: { backgroundColor: '#111', borderRadius: 8, padding: 14, alignItems: 'center', marginBottom: 12 },
  ...
  badge: { backgroundColor: '#eee', borderRadius: 12, paddingHorizontal: 8, paddingVertical: 3, fontSize: 12, color: '#555' },
